...
async def ingest_submission(
    ...
    submission_doc["user_id"] = user_id
    submission_doc["id"] = str(uuid4())
    submission_doc["timestamp"] = datetime.utcnow()
...
async def list_submissions(
    ...
    Returns summaries (currently full objects, but could be slimmed down).
    """
    user_id = current_user["user_id"]
